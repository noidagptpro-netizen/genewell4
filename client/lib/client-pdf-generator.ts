...
function renderCoverPage(ctx: PDFContext, bundle: PDFDataBundle, language: "en" | "hi" = "en"): void {
  ...
  const estimatedSleepHours = (profile.sleepScore * 8 / 100).toFixed(1);
...
function renderTableOfContents(ctx: PDFContext, bundle: PDFDataBundle, language: "en" | "hi" = "en"): void {
  ...
  const entries: TocEntry[] = [
    ...
    },
    {
      ...
      show: (tier === "premium" || tier === "coaching") && (bundle.mealPlan?.days?.length > 0),
    },
    {
      num: "07",
    ...
    },
    {
      ...
      show: (tier === "premium" || tier === "coaching"),
    },
    {
      num: "11",
...
function renderWellnessBaseline(ctx: PDFContext, bundle: PDFDataBundle, language: "en" | "hi" = "en"): void {
  ...
  const waterLiters = profile.waterLiters || profile.waterIntake || 1.5;
  const hydrationScore = Math.max(25, Math.min(90, Math.round((waterLiters / 2.1) * 85)));
  const scores = [
    {
      icon: "⚡", label: l("Energy Level", "ऊर्जा स्तर"), score: profile.energyScore,
      note: profile.energyScore < 40
        ? l(`Linked to ${profile.sleepHours < 7 ? `${profile.sleepHours} hrs sleep` : "low hydration"} + ${waterLiters < 2 ? `${waterLiters}L water` : "stress"}`, `नींद और जलयोजन से जुड़ा`)
    ...
    },
    {
      ...
      note: profile.stressScore < 50
        ? l("Moderate stress + anxiety pattern — specific tools provided", "मध्यम तनाव + चिंता पैटर्न — विशिष्ट उपकरण प्रदान किए गए")
        : profile.stressScore < 75 ? l("Manageable — daily techniques will prevent burnout", "प्रबंधनीय — तकनीकें बर्नआउट रोकेंगी") : l("Well-managed — continue practices", "अच्छी तरह प्रबंधित"),
    },
    {
...
function renderHealthScoreDashboard(ctx: PDFContext, bundle: PDFDataBundle, language: "en" | "hi" = "en"): void {
  ...
  const sleepHours = profile.sleepHours || 6;
  ...
  const mealTimingScore = Math.min(100, Math.round((mealFreq / 3) * 100));
  const waterLiters = profile.waterLiters || profile.waterIntake || 1.5;
  const hydrationScore = Math.max(25, Math.min(90, Math.round((waterLiters / 2.1) * 85)));
  ...
  const waterTarget = Math.round(profile.weightKg * 0.033 * 10) / 10;
  const hydRootCause = waterLiters < 1.5
    ? l(`${waterLiters}L/day (need ${waterTarget}L)`, `${waterLiters}L/दिन (${waterTarget}L चाहिए)`)
    : l(`${waterLiters}L/day — near target`, `${waterLiters}L/दिन — लक्ष्य के करीब`);

  const mealRootCause = mealFreq < 3
    ? l(`${mealFreq} meals (need 3)`, `${mealFreq} भोजन (3 चाहिए)`)
    : l("Good meal frequency", "भोजन आवृत्ति अच्छी");

  const stressRootCause = (profile.stressScore || 50) > 60
    ? l("Moderate-high stress + lifestyle", "मध्यम-उच्च तनाव")
  ...
  const dims = [
    ...
    { name: l("Meal Timing", "भोजन समय"), score: mealTimingScore, gap: 100 - mealTimingScore, root: mealRootCause },
...
function renderMetabolicCalculations(ctx: PDFContext, bundle: PDFDataBundle, language: "en" | "hi" = "en"): void {
  ...
  const steps = [
    ...
    },
    {
      ...
      formula: l(`BMR × 1.375 (Lightly Active) = ${bmr} × 1.375 = ${tdee} kcal/day`,
                  `BMR × 1.375 = ${bmr} × 1.375 = ${tdee} kcal/दिन`),
      note: wantsGain
  ...
  // ── Macro Targets table ──
  const goalCals = (goalCalMin + goalCalMax) / 2;
  const proteinPct = Math.round(((profile.proteinGrams * 4) / goalCals) * 100);
  const carbsPct = Math.round(((profile.carbsGrams * 4) / goalCals) * 100);
  const fatsPct = Math.round(((profile.fatsGrams * 9) / goalCals) * 100);
  ...
  drawPremiumTable(ctx,
    ...
    [
      [l("Protein", "प्रोटीन"), `${profile.proteinGrams}g`, `${proteinPct}%`,
      ...
      [l("Carbohydrates", "कार्बोहाइड्रेट"), `${profile.carbsGrams}g`, `${carbsPct}%`,
        l("Oats, brown rice, millets, sweet potato, banana", "ओट्स, ब्राउन राइस, बाजरा, शकरकंद, केला")],
      [l("Healthy Fats", "स्वास्थ्य वसा"), `${profile.fatsGrams}g`, `${fatsPct}%`,
...
function renderDecisionEngine(ctx: PDFContext, bundle: PDFDataBundle, language: "en" | "hi" = "en"): void {
  ...
  if (sleepLow) {
    ...
    chainItems = [
      ...
      { text: l("Thyroid Suppression", "थायरॉइड दमन"), type: "Metabolic" },
    ...
    chainItems = [
      ...
      { text: l("Fat Gain + Inflammation", "वसा बढ़ना + सूजन"), type: "Symptoms" },
    ];
  }
  ...
  const waterVal = profile.waterLiters || profile.waterIntake || 1.2;
  ...
  const dimScores = [
    ...
    { name: l("Hydration", "जलयोजन"), score: Math.max(25, Math.round((waterVal / 2.1) * 85)) },
    { name: l("Meal Frequency", "भोजन आवृत्ति"), score: Math.min(100, Math.round((mealFreqVal / 3) * 100)) },
...
function renderMetabolicProfile(ctx: PDFContext, bundle: PDFDataBundle, language: "en" | "hi" = "en"): void {
  ...
  const steps = [
    ...
    },
    {
      ...
      formula: l(`BMR × ${(tdee / bmr).toFixed(3)} (${profile.workSchedule || "Lightly Active"}) = ${bmr} × ${(tdee / bmr).toFixed(3)} = ${tdee} kcal/day`, `BMR × ${(tdee / bmr).toFixed(3)} = ${tdee} kcal/दिन`),
...
function renderSleepProtocol(ctx: PDFContext, bundle: PDFDataBundle, language: "en" | "hi" = "en"): void {
  ...
  addHeaderSection(ctx, l("07 · Sleep & Energy Protocol", "07 · नींद और ऊर्जा प्रोटोकॉल"), l(`Sleep Score: ${profile.sleepScore}/100 · Target: ${Math.max(profile.sleepHours || 6, 7)}-9 hrs nightly`, `नींद स्कोर: ${profile.sleepScore}/100 · लक्ष्य: ${Math.max(profile.sleepHours || 6, 7)}-9 घंटे रात`));
...
function renderStressManagement(ctx: PDFContext, bundle: PDFDataBundle, language: "en" | "hi" = "en"): void {
  ...
  addHeaderSection(ctx, l("08 · Anxiety & Mood Management", "08 · चिंता और मनोदशा प्रबंधन"), l(`Stress Score: ${profile.stressScore}/100 · ${profile.stressScore > 60 ? "HIGH — act immediately" : profile.stressScore > 40 ? "MODERATE — daily tools required" : "LOW — maintenance mode"}`, `तनाव स्कोर: ${profile.stressScore}/100 · ${profile.stressScore > 60 ? "उच्च — तुरंत कार्य करें" : profile.stressScore > 40 ? "मध्यम — दैनिक उपकरण आवश्यक" : "निम्न — रखरखाव मोड"}`));
...
function renderSupplementStrategy(ctx: PDFContext, bundle: PDFDataBundle, language: "en" | "hi" = "en"): void {
  ...
  if (isVeg) {
    suppRows.push([
      ...
      l("Vegetarian diet = B12 deficiency risk → fatigue, nerve damage", "शाकाहारी आहार = B12 कमी जोखिम → थकान, तंत्रिका क्षति"),
...
function renderLabTests(ctx: PDFContext, bundle: PDFDataBundle, language: "en" | "hi" = "en"): void {
  ...
  addHeaderSection(ctx, l("13 · Recommended Lab Tests", "13 · अनुशंसित लैब परीक्षण"), l(`${profile.name}'s prioritized panel — thyroid & full screening`, `${profile.name} का प्राथमिकता पैनल — थायरॉइड और पूर्ण स्क्रीनिंग`));
  ...
  // HIGH priority
  if (profile.sleepScore < 65 || (profile.dietaryPreference || "").toLowerCase().includes("veg")) {
    highTests.push({
      name: l("Vitamin B12 & Folate", "विटामिन B12 और फोलेट"),
      cost: "₹500–900",
      when: l("Week 0", "सप्ताह 0"),
      why: l("B12 deficiency causes fatigue, nerve damage, poor mood. Especially critical for vegetarians.", "B12 कमी से थकान, तंत्रिका क्षति, खराब मूड। शाकाहारियों के लिए विशेष रूप से गंभीर।"),
      tier: "HIGH",
    });
  }
  ...
  if (hasPCOS && isFemLab) {
    highTests.push({
      ...
      cost: "₹800–1,500",
      when: l("Day 2-5 of cycle", "चक्र के दिन 2-5"),
      why: l("PCOS requires hormone baseline for treatment and monitoring.", "PCOS में हार्मोन बेसलाइन उपचार और निगरानी के लिए जरूरी।"),
      tier: "HIGH",
    });
  }
    ...
    drawPremiumTable(ctx,
      ...
      [
        ...
        [l("Phase 1\nWeeks 1–4","चरण 1\nसप्ताह 1-4"),
          ...
          isVeg
            ? l("★WARN★ Vegetarian: use algae-based omega-3 (not fish). Critical for brain + cardiovascular.","★WARN★ शाकाहारी: मछली की जगह शैवाल-आधारित ओमेगा-3 का उपयोग करें।")
  ...
  drawPhaseBlocks(ctx, [
    ...
    },
    {
      ...
      items: [
        ...
        ...((tier === "premium" || tier === "coaching") ? [l("Begin Phase 1 supplements (Vitamin D3 + Magnesium first)", "चरण 1 सप्लीमेंट शुरू करें (विटामिन D3 + मैग्नीशियम पहले)")] : []),
      ],
    },
    {
...
export async function generatePDFFromBundle(rawBundle: PDFDataBundle, language: "en" | "hi" = "en"): Promise<{ blob: Blob; filename: string }> {
  ...
  if ((tier === "premium" || tier === "coaching") && activeModules.includes("meal_plan")) {
  ...
  if ((tier === "premium" || tier === "coaching") && activeModules.includes("gut_health")) {
    renderDigestiveHealth(ctx, bundle, pdfLanguage);
  }

  // 11 · Skin Health — premium/coaching (if skin concerns)
  if ((tier === "premium" || tier === "coaching") && activeModules.includes("skin_health") && profile.skinConcerns?.length > 0) {
    renderSkinHealth(ctx, bundle, pdfLanguage);
  }

  // 12 · Supplement Strategy — premium/coaching
  if ((tier === "premium" || tier === "coaching") && activeModules.includes("supplements")) {
  ...
  if (tier === "coaching" && activeModules.includes("coaching_edition")) {
